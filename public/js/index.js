...
(function () {
  ...
  function renderDataTable() {
    ...
    if (underlying == 1) {
      worksheet.getUnderlyingDataAsync({ maxRows: max_no_records }).then(function (underlying) {
        ...
        var data = [];
        var column_filter_name = tableau.extensions.settings.get("column_filter_name").split("|");
        var column_names = tableau.extensions.settings.get("column_names").split("|");
        column_names = column_names.filter(elem=>column_filter_name.includes(elem));
        ...
        var column_filter = tableau.extensions.settings.get("column_filter").split("|");
        column_order = column_order.filter(elem=>column_filter.includes(elem));
        var tableData = makeArray(column_order.length,underlying.totalRowCount);
  ...
  function configure() {
    ...
    //const popupUrl = '/dialog.html';
    //window.alert('configure called ' + popupUrl);
    let input = "";
    ...
    tableau.extensions.ui.displayDialogAsync(popupUrl, input, { height: 540, width: 800 }).then((closePayload) => {
      ...
      switch (error.errorCode) {
        case tableau.ErrorCodes.DialogClosedByUser:
          ...
          break;
        default:
        //window.alert('default!:');// + error.errorCode);
