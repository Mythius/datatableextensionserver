...
(function () {
  ...
  function buildDialog() {
    ...
    if (column_order != undefined && column_order.length > 0) {
      ...
      var column_filtered_array = tableau.extensions.settings.get("column_filter").split("|");
      $("#sort-it ol").text("");
      for (var i = 0; i < column_names_array.length; i++) {
        //alert(column_names_array[i] + " : " + column_order_array[i]);
        let checked = column_filtered_array.includes(column_order_array[i])?'checked':'';
        $("#sort-it ol").append("<li><input type='checkbox'"+checked+"><div class='input-field'><input id='" + column_names_array[i] + "' type='text' col_num=" + column_order_array[i] + "><label for=" + column_names_array[i] + "'>" + column_names_array[i] + "</label></div></li>");
    ...
    if (underlying == 1) {
      ...
      worksheet.getUnderlyingDataAsync({ maxRows: 1 }).then(function (sumdata) {
        ...
        worksheetColumns.forEach(function (current_value) {
          ...
          $("#sort-it ol").append("<li><input type='checkbox' checked><div class='input-field'><input id='" + current_value.fieldName + "' type='text' col_num=" + counter + "><label for=" + current_value.fieldName + "'>" + current_value.fieldName + "</label></div></li>");
          // $("#sort-it ol").append("");
          counter++;
        });
      });
  ...
  function saveButton() {
    ...
    let selected_column_nums = [...$('#sort-it input[type=checkbox]:checked')].map(e=>e.nextSibling.firstChild.getAttribute('col_num')).join('|');
    tableau.extensions.settings.set("column_filter",selected_column_nums);

    let selected_column_names = [...$('#sort-it input[type=checkbox]:checked')].map(e=>e.nextSibling.firstChild.getAttribute('id')).join('|');
    tableau.extensions.settings.set("column_filter_name",selected_column_names);
    ...
    var column_name = "";
    var counter = 0;
    $("#sort-it").find("input[type='text']").each(function (column) {
      // This handles the column order
      if (counter == 0) {
        column_order = $(this).attr("col_num");
      } else {
        // if($(this).attr("col_num")=='3') return; //added
